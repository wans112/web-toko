...
export default function OrderClient({ onStartShopping }) {
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      {orders.length === 0 ? (
        <Card>
          ...
          >
            <Button type="primary" onClick={onStartShopping}>Mulai Berbelanja</Button>
          </Empty>
        </Card>
      ) : (
              ...
              <ul className="divide-y divide-gray-100">
                {(selectedOrder.items || []).map(item => (
                  <li key={item.id} className="py-3 flex flex-col gap-2">
                    ...
                    {item.discount_amount > 0 && (
                      <div className="flex flex-wrap gap-2 items-center">
                        <span className="text-xs text-green-600 font-semibold">Hemat {formatPrice(item.discount_amount)}</span>
                      </div>
                    )}
                  </li>
                ))}
