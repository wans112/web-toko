...
export default function ManagementDiscountPercentage() {
            ...
            return (
              <div key={p.id} className="mb-2">
                ...
                {p.units.map(unit => {
                  ...
                  const afterDisc = showDiscount && price != null ? price - (price * record.value / 100) : price;
                  return (
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (discountJson.success) {
        // Filter percentage discounts and map products/units with prices
        const dataDiskon = (discountJson.data || [])
          .filter(d => d.value_type === 'percentage')
          .map(diskon => {
  ...
  const handleAdd = () => {
    setEditing(null);
    form.resetFields();
    form.setFieldsValue({ type: 'product', value_type: 'percentage' });
  ...
  return (
    <div className="w-full">
      <div className="mb-4 flex justify-between items-center">
        <h2 className="text-lg font-semibold m-0">Manajemen Diskon Persentase</h2>
        <Button type="primary" onClick={handleAdd}>
          <PlusOutlined /> Tambah Diskon Persentase
        </Button>
      </div>
      ...
      <Modal
        title={
          <Space>
            <PercentageOutlined />
            {editing ? "Edit Diskon Persentase" : "Tambah Diskon Persentase"}
          </Space>
        }
        open={modalOpen}
        onOk={handleModalOk}
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item
            label="Persentase (%)"
            name="value"
            rules={[{ required: true, type: "number", min: 0, max: 100, message: "Persentase 0-100" }]}
          >
            <InputNumber 
              min={0} 
              max={100} 
